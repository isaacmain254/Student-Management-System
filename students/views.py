...
def dashboard(request):
    ...
    years = students.values('year_of_study').annotate(
        total_students=Count('id'))
    student_counts = [year['total_students'] for year in years]

    context = {'students': students, 'males': males, 'females': females,
               'in_session': in_session, 'student_counts': student_counts}
...
def students_list(request):
    ...
    # search for student
    if search_query:
        students = Student.objects.filter(Q(first_name__icontains=search_query) | Q(last_name__icontains=search_query) | Q(
            registration_no__icontains=search_query) | Q(year_of_study__icontains=search_query))
...
def edit_details(request, student_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"{student.first_name} {student.last_name} details updated successfully")
            return redirect('students:students_list')
        # error flush message
        messages.error(request, "Please correct the errors!")
